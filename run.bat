...
echo    ^> Kiem tra config.json...
python -c "import json; config=json.load(open('config/config.json','r',encoding='utf-8')); channel=config.get('channels',{}).get('default_channel',''); exit(0 if channel and channel not in ['', '@your_channel_here'] else 1)" 2>nul
if errorlevel 1 (
    echo.
...
python -c "import sys; import os; sys.path.insert(0, 'src'); from utils.logger import setup_detailed_logging; import json; config = json.load(open('config/config.json', 'r', encoding='utf-8')); setup_detailed_logging(config.get('logging', {})); print('[OK] He thong logging da san sang')" 2>nul
if errorlevel 1 (
...
if "%choice%"=="2" (
    ...
    if not "!new_channel!"=="" (
        ...
        python -c "import json; config=json.load(open('config/config.json','r',encoding='utf-8')); config['channels']['default_channel']='!new_channel!'; config['channels']['use_default_channel']=True; json.dump(config,open('config/config.json','w',encoding='utf-8'),indent=2,ensure_ascii=False)" 2>nul
        if errorlevel 1 (
...
if "%choice%"=="3" (
    ...
    if not "!max_msg!"=="" (
        ...
        if "!max_msg!"=="0" (
            python -c "import json; config=json.load(open('config/config.json','r',encoding='utf-8')); config['scanning']['max_messages']=None; json.dump(config,open('config/config.json','w',encoding='utf-8'),indent=2,ensure_ascii=False)" 2>nul
        ) else (
            python -c "import json; config=json.load(open('config/config.json','r',encoding='utf-8')); config['scanning']['max_messages']=int('!max_msg!'); json.dump(config,open('config/config.json','w',encoding='utf-8'),indent=2,ensure_ascii=False)" 2>nul
        )
        if errorlevel 1 (
...
if "%choice%"=="4" (
    ...
    if "!file_choice!"=="1" (
        python -c "import json; config=json.load(open('config/config.json','r',encoding='utf-8')); config['scanning']['file_types']={'documents':True,'photos':True,'videos':True,'audio':True}; json.dump(config,open('config/config.json','w',encoding='utf-8'),indent=2,ensure_ascii=False)" 2>nul
        echo [OK] Da chon tat ca loai file
    )
    if "!file_choice!"=="2" (
        python -c "import json; config=json.load(open('config/config.json','r',encoding='utf-8')); config['scanning']['file_types']={'documents':True,'photos':False,'videos':False,'audio':False}; json.dump(config,open('config/config.json','w',encoding='utf-8'),indent=2,ensure_ascii=False)" 2>nul
        echo [OK] Da chon chi documents
    )
    if "!file_choice!"=="3" (
        python -c "import json; config=json.load(open('config/config.json','r',encoding='utf-8')); config['scanning']['file_types']={'documents':False,'photos':True,'videos':True,'audio':False}; json.dump(config,open('config/config.json','w',encoding='utf-8'),indent=2,ensure_ascii=False)" 2>nul
        echo [OK] Da chon photos va videos
    )
    if "!file_choice!"=="4" (
        python -c "import json; config=json.load(open('config/config.json','r',encoding='utf-8')); config['scanning']['file_types']={'documents':False,'photos':False,'videos':False,'audio':True}; json.dump(config,open('config/config.json','w',encoding='utf-8'),indent=2,ensure_ascii=False)" 2>nul
...
if "%choice%"=="5" (
    ...
    if "!format_choice!"=="1" (
        python -c "import json; config=json.load(open('config/config.json','r',encoding='utf-8')); config['output']['formats']['csv']['enabled']=True; config['output']['formats']['json']['enabled']=True; config['output']['formats']['excel']['enabled']=True; json.dump(config,open('config/config.json','w',encoding='utf-8'),indent=2,ensure_ascii=False)" 2>nul
        echo [OK] Da chon tat ca dinh dang
    )
    if "!format_choice!"=="2" (
        python -c "import json; config=json.load(open('config/config.json','r',encoding='utf-8')); config['output']['formats']['csv']['enabled']=True; config['output']['formats']['json']['enabled']=False; config['output']['formats']['excel']['enabled']=False; json.dump(config,open('config/config.json','w',encoding='utf-8'),indent=2,ensure_ascii=False)" 2>nul
        echo [OK] Da chon chi CSV
    )
    if "!format_choice!"=="3" (
        python -c "import json; config=json.load(open('config/config.json','r',encoding='utf-8')); config['output']['formats']['csv']['enabled']=False; config['output']['formats']['json']['enabled']=True; config['output']['formats']['excel']['enabled']=False; json.dump(config,open('config/config.json','w',encoding='utf-8'),indent=2,ensure_ascii=False)" 2>nul
        echo [OK] Da chon chi JSON
    )
    if "!format_choice!"=="4" (
        python -c "import json; config=json.load(open('config/config.json','r',encoding='utf-8')); config['output']['formats']['csv']['enabled']=False; config['output']['formats']['json']['enabled']=False; config['output']['formats']['excel']['enabled']=True; json.dump(config,open('config/config.json','w',encoding='utf-8'),indent=2,ensure_ascii=False)" 2>nul
...
python -c "import json; config=json.load(open('config/config.json','r',encoding='utf-8')); print('   ', config.get('channels',{}).get('default_channel','Chua cau hinh'))" 2>nul
echo.
